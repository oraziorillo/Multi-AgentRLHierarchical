...
class ParticleEnv(MultiAgentEnv):
    ...
    def __init__(self, n_listeners=1, n_landmarks=3, collide=False, render_enable=False):
        ...
        for agent in self.world.agents:
            ...
            if not agent.silent:
                ...
                c_action_space = spaces.Discrete(n_landmarks)
    ...
    def _exec_step(self, action, update_comm):
        ...
        for agent in self.world.agents:
            ...
            if agent.name.startswith('manager'):
                env_reward = self._reward(agent)
                if update_comm:
                    obs[agent.name] = self._obs(agent)
                    rewards[agent.name] = env_reward

            else:
                obs[agent.name] = self._obs(agent)
                agent.update_obs(obs[agent.name])
                # rewards[agent.name] = env_reward + agent.compute_reward()
                rewards[agent.name] = agent.compute_reward()

            dones[agent.name] = self._done(agent)
        ...
        dones['__all__'] = dones['manager_agent']
    ...
    def _done(self, agent):
        ...
        # a manager is done when all the workers are close to their goal
        else:
            done = True
            for goal in agent.goal.values():
